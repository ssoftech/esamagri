        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
						...
						<!--  <li class="treeview">
						 ...
						 <li class="treeview">
                            ...
                            <ul class="treeview-menu">
                                ...
                                <!-- <a href="pages/charts/inline.html"><i class="fa fa-angle-double-right"></i>Low Stock Products</a></li>  -->
                            </ul>
                        </li>
						 <li class="treeview">
                            <a href="#">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        <li><a href="Dashbord.jsp"><i class="fa fa-dashboard"></i> Home</a></li>
...
ArrayList<Product_bean> ar=Product_Modeldb.show(name1);
if(ar!=null)
{%><div id="show"><h4><center><table border="1" id="product"><tr><th>PRODUCT_NAME</th><th>BRAND_NAME</th><th>MSRP</th>
<th>SALES_PRICE</th><th>SUPPLIER_ID</th><th>PRODUCT_STATUS</th><th>SHIPPING_COST</th><th>PRODUCT_UPDATED</th><th>IMAGE</th><th>Action</th>
</center></h4></tr>
...
background:#72afd2;
	...
	<% Iterator<Product_bean> it=ar.iterator();
	int cnt=0;
	while(it.hasNext())
	{
		Product_bean ob=it.next();
		System.out.println("product_id "+ob.getPRODUCT_ID());
	    cnt++;
	    System.out.println("product_name "+ob.getPRODUCT_NAME());
	    System.out.println("brand_name "+ob.getBRAND_NAME());
		%>
		<tr>
		...
		<td><%=ob.getPRODUCT_STATUS() %></td>
		<td><%=ob.getSHIPPING_COST() %></td>
		<td><%=ob.getPRODUCT_UPDATED() %></td>
		<% Connection con=Db.Condb();
		 String sql="Select IMAGE_ID from image where  PRODUCT_ID=? ";
		 PreparedStatement ps=con.prepareStatement(sql);
		 ps.setInt(1,ob.getPRODUCT_ID());
		 ResultSet rs=ps.executeQuery();
		 if(rs.next()){ int image_id=rs.getInt(1);
		  System.out.println("in main page 1st "+image_id);%>
		 <td><div id="image<%=cnt%>"><img src="Product_Image?product_id=<%=ob.getPRODUCT_ID()%>&image_id=<%=image_id%>" height="70" width="70"></img>
		</div>
		<% int count=Product_Modeldb.CountImageByProductId(ob.getPRODUCT_ID());
		 int p=1;
		
		 String sql11="Select IMAGE_ID from image where  PRODUCT_ID=? and IMAGE_ID!=?";
		 ...
		 while(rs11.next())
		 {
		  int image_id1=rs11.getInt(1);
		  System.out.println("in main page other"+image_id1);
		  %><a href="#" onClick="change_image(<%=ob.getPRODUCT_ID()%>,<%=image_id1%>,<%=cnt%>)"><%=p%></a>
		  <% p++; }%>
		  <a href="#" onClick="change_image(<%=ob.getPRODUCT_ID()%>,<%=image_id%>,<%=cnt%>)"><%=p%></a>
		  </td><%} %>
		
		<td align="center">
		<a href="edit.jsp?pid=<%=ob.getPRODUCT_ID()%>" >Edit</a><hr>
		<a href="delete1.jsp?pid=<%=ob.getPRODUCT_ID()%>">Delete</a><hr>
		</td>
	   </tr>
