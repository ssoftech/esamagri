...
import bean.pending_order_bean;

import db.Db;

public class pending_order_modeldb {
	public static ArrayList<pending_order_bean> pendingorderdetails()	{
		Connection con=null;
		con=Db.Condb();
	ArrayList<pending_order_bean> ar=new ArrayList<pending_order_bean>();


	try {
		pending_order_bean ob=null;
		con=Db.Condb();
		...
		while(rs.next())
		{
			...
			ob=new pending_order_bean(oRDER_DETAILS_ID, aDDRESS, oRDER_AMOUNT, oRDER_DATE, pAYMENT_TYPE_ID, oRDER_STATUS_ID, email, name, supplier_id);
			ar.add(ob);
		}
